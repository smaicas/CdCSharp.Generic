...
internal sealed partial class CacheEntry : ICacheEntry
{
    ...
    private static readonly AsyncLocal<CacheEntry?> _current = new();
    ...
    DateTimeOffset? ICacheEntry.AbsoluteExpiration
    {
        get
        {
            ...
            TimeSpan offset = new(_absoluteExpirationOffsetMinutes * TimeSpan.TicksPerMinute);
    ...
    {
        get => AbsoluteExpirationRelativeToNow.Ticks == 0 ? null : AbsoluteExpirationRelativeToNow;
        set
        {
            ...
            AbsoluteExpirationRelativeToNow = value.GetValueOrDefault();
        }
    }
    ...
    internal long Size { get; private set; } = NotSet;

    long? ICacheEntry.Size
    {
        get => Size < 0 ? null : Size;
        set
        {
            if (value < 0)
            {
    ...
    private static void ExpirationTokensExpired(object obj)
    {
        ...
        {
            CacheEntry entry = (CacheEntry)state!;
